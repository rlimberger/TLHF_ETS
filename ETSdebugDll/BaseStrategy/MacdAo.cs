    ...
    public class MacdAo : Script
    {
        ...
        private void Entry(int bar)
        {
            if (LongPos.Count == 0 && ShortPos.Count == 0 && _macdB[bar] > _macdG[bar] && _macdB[bar - 1] < 0 
                && _macdB[bar] < 0 && _ao[bar - 1] < 0 && _ao[bar] > 0 && _parabolic[bar] < Candles.LowSeries[bar])
            {
                _long = BuyAtMarket(bar + 1, 1, "Long");
                _longStopLoss = _parabolic[bar];
            }
            if (LongPos.Count == 0 && ShortPos.Count == 0 && _macdB[bar] < _macdG[bar] && _macdB[bar - 1] > 0
                && _macdB[bar] > 0 && _ao[bar - 1] > 0 && _ao[bar] < 0 && _parabolic[bar] > Candles.HighSeries[bar])
            {
        ...
        public override void GetAttributesStratetgy()
        {
            ...
            DesParamStratetgy.LinkFullDescription = "https://etstrading.ru/roboti#rec357280956";
            DesParamStratetgy.NameStrategy = "MacdAo";
        }
    }
}
