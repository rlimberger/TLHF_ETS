...
{
    public class DarkCloudCover : Script
    {
        ...
        private string DetectFormation(int bar)
        {
            ...
            if (Candles.OpenSeries[bar - 1] > Candles.CloseSeries[bar - 1] && Candles.OpenSeries[bar] < Candles.CloseSeries[bar]
                ...
                && Candles.CloseSeries[bar] > Candles.LowSeries[bar - 1] + (Candles.HighSeries[bar - 1] - Candles.LowSeries[bar - 1]) / 2
                && Candles.CloseSeries[bar] < Candles.OpenSeries[bar - 1])
            {
                PlotArea("Buy dark cloud", bar, new UserChartPropModel() { ColorLine = Color.LightGreen });
                return "buy";
            }
            if (Candles.OpenSeries[bar - 1] < Candles.CloseSeries[bar - 1] && Candles.OpenSeries[bar] > Candles.CloseSeries[bar]
                && Candles.OpenSeries[bar] > Candles.CloseSeries[bar - 1] 
                && Candles.CloseSeries[bar] < Candles.HighSeries[bar - 1] - (Candles.HighSeries[bar - 1] - Candles.LowSeries[bar - 1]) / 2
                && Candles.CloseSeries[bar] > Candles.OpenSeries[bar - 1])
            {
                PlotArea("Sell dark cloud", bar, new UserChartPropModel() { ColorLine = Color.OrangeRed });
                return "sell";
            }
            return "";
        }
        ...
        public override void GetAttributesStratetgy()
        {
            ...
            DesParamStratetgy.NameStrategy = "DarkCloudCover";
        }
    }
}
